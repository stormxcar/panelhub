...
export default async function Home() {
  ...
  return (
    <main>
      ...
      <section id="bao-gia" className="section pricing reveal"><div><p className="kicker">Ngân sách minh bạch</p><h2>Báo giá nhà tiền chế panel tham khảo theo m2</h2><p>Giá thay đổi theo vật tư, nền móng, chiều cao, loại lõi panel và mức hoàn thiện.</p></div><div className="price-table" role="group" aria-label="Các gói báo giá tham khảo">{managedPricing.map(([name, price, note], index) => <article className={`price-row ${index === 1 ? "is-recommended" : ""}`} key={name}><div className="price-name">{index === 1 ? <span className="price-badge">Khuyến nghị</span> : null}<strong>{name}</strong>{index === 1 ? <small>Phương án cân bằng chi phí, khả năng cách nhiệt và mức hoàn thiện.</small> : null}</div><span>{price}</span><p>{note}</p></article>)}</div></section>
