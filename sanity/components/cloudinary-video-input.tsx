...
export function CloudinaryVideoInput(props: StringInputProps) {
  ...
  const upload = async (file?: File) => {
    ...
    if (!signatureResponse.ok) { setStatus(`Không thể lấy quyền upload video (${signatureResponse.status}): ${await errorDetail(signatureResponse)}`); return; }
    ...
    if (!response.ok) { setStatus(`Upload video thất bại (${response.status}): ${await errorDetail(response)}`); return; }
