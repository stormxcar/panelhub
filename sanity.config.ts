...
const imageItemPreview = {
  select: { title: "title", label: "label", imageCloudinaryUrl: "imageCloudinaryUrl", imageUrl: "imageUrl" },
  prepare({ title, label, imageCloudinaryUrl, imageUrl }: { title?: string; label?: string; imageCloudinaryUrl?: string; imageUrl?: string }) {
    const previewUrl = imageCloudinaryUrl || imageUrl;
    return {
...
const cloudinaryImageUploadField = () => defineField({ name: "imageCloudinaryUrl", title: "Ảnh mới tải lên Cloudinary", description: "Ảnh tại đây được ưu tiên hiển thị trên website. Chọn file để upload hoặc dán URL Cloudinary.", type: "string", components: { input: CloudinaryImageInput } });
...
export default defineConfig({
  ...
  schema: { types: [
    ...
    defineType({ name: "homePage", title: "Trang chủ", type: "document", initialValue: { heroTitle: "Nhà tiền chế tấm panel", heroDescription: "Giải pháp xây dựng hiện đại, triển khai gọn và phù hợp nhu cầu sử dụng thực tế." }, fieldsets: [
      ...
      defineField({ name: "heroImages", title: "4 ảnh công trình", description: "Ảnh đầu tiên là ảnh chính khi website mở. Upload Cloudinary được ưu tiên hiển thị trên website.", type: "array", fieldset: "hero", of: [defineArrayMember({ type: "object", preview: imageItemPreview, fields: [defineField({ name: "label", title: "Nhãn ảnh", type: "string" }), cloudinaryImageUploadField(), defineField({ name: "image", title: "Ảnh đã tải lên Sanity (cũ)", type: "image", options: { hotspot: true } }), defineField({ name: "imageUrl", title: "URL Cloudinary cũ", type: "string", components: { input: CloudinaryImageInput } })] })] }),
      ...
      defineField({ name: "services", title: "6 card dịch vụ", description: "Mỗi card gồm tiêu đề, nhãn, mô tả và ảnh preview/upload.", type: "array", fieldset: "services", of: [defineArrayMember({ type: "object", preview: imageItemPreview, fields: [defineField({ name: "title", title: "Tiêu đề", type: "string" }), defineField({ name: "tag", title: "Nhãn", type: "string" }), defineField({ name: "description", title: "Mô tả", type: "text" }), cloudinaryImageUploadField(), defineField({ name: "image", title: "Ảnh đã tải lên Sanity (cũ)", type: "image", options: { hotspot: true } }), defineField({ name: "imageUrl", title: "URL Cloudinary cũ", type: "string", components: { input: CloudinaryImageInput } })] })] }),
      defineField({ name: "anatomy", title: "Nội dung và ảnh cấu tạo", type: "object", fieldset: "anatomy", fields: [defineField({ name: "title", title: "Tiêu đề", type: "string" }), defineField({ name: "description", title: "Mô tả", type: "text" }), cloudinaryImageUploadField(), defineField({ name: "image", title: "Ảnh đã tải lên Sanity (cũ)", type: "image", options: { hotspot: true } }), defineField({ name: "imageUrl", title: "URL Cloudinary cũ", type: "string", components: { input: CloudinaryImageInput } }), defineField({ name: "panelTypes", title: "Các loại panel", type: "array", of: [defineArrayMember({ type: "object", fields: [defineField({ name: "name", title: "Tên loại panel", type: "string" }), defineField({ name: "description", title: "Mô tả", type: "text" })] })] })] }),
      defineField({ name: "processFeature", title: "Ảnh chính quy trình", type: "object", fieldset: "process", fields: [cloudinaryImageUploadField(), defineField({ name: "image", title: "Ảnh đã tải lên Sanity (cũ)", type: "image", options: { hotspot: true } }), defineField({ name: "imageUrl", title: "URL Cloudinary cũ", type: "string", components: { input: CloudinaryImageInput } })] }),
      defineField({ name: "processSteps", title: "7 ảnh và bước triển khai", type: "array", fieldset: "process", of: [defineArrayMember({ type: "object", preview: imageItemPreview, fields: [defineField({ name: "title", title: "Tên bước", type: "string" }), defineField({ name: "description", title: "Mô tả", type: "text" }), cloudinaryImageUploadField(), defineField({ name: "image", title: "Ảnh đã tải lên Sanity (cũ)", type: "image", options: { hotspot: true } }), defineField({ name: "imageUrl", title: "URL Cloudinary cũ", type: "string", components: { input: CloudinaryImageInput } })] })] }),
      defineField({ name: "projects", title: "6 dự án", type: "array", fieldset: "projects", of: [defineArrayMember({ type: "object", preview: imageItemPreview, fields: [defineField({ name: "title", title: "Tiêu đề", type: "string" }), defineField({ name: "category", title: "Danh mục", type: "string" }), defineField({ name: "description", title: "Mô tả", type: "text" }), cloudinaryImageUploadField(), defineField({ name: "image", title: "Ảnh đã tải lên Sanity (cũ)", type: "image", options: { hotspot: true } }), defineField({ name: "imageUrl", title: "URL Cloudinary cũ", type: "string", components: { input: CloudinaryImageInput } })] })] }),
